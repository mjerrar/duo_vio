    ...
    if (i + 1 < 5) {
      ...
      if (lastv > 0) {
        if (lastc == 0) {
          ...
          iy = 0;
          i31 = i_ip1 + 6 * (lastc - 1);
          for (itemp = i_ip1; itemp <= i31; itemp += 6) {
            ix = i_i;
            smax = 0.0;
            pvt = (itemp + lastv) - 1;
            for (k = itemp; k <= pvt; k++) {
...
// Arguments    : const double c_cameraparams_CameraParameters[3]
...
// Return Type  : void
//
void initializePoint(const double c_cameraparams_CameraParameters[3], const
                     ...
                     double m_out[3])
{
  ...
  for (i = 0; i < 3; i++) {
    pos[i] = 0.0;
    pos[3 + i] = cameraparams_r_lr[i];
    for (rankR = 0; rankR < 3; rankR++) {
      b_cameraparams_R_lr[rankR + 3 * i] = cameraparams_R_lr[i + 3 * rankR];
    }
  }

  QuatFromRotJ(b_cameraparams_R_lr, dv4);
  for (i = 0; i < 4; i++) {
    rot[i] = iv3[i];
    rot[4 + i] = dv4[i];
  }

  undistortPoint(z_l, c_cameraparams_CameraParameters,
                 d_cameraparams_CameraParameters,
                 e_cameraparams_CameraParameters, z_u_l);
  undistortPoint(z_r, f_cameraparams_CameraParameters,
                 g_cameraparams_CameraParameters,
                 h_cameraparams_CameraParameters, z_u_r);
  ml[0] = z_u_l[0];
  ml[1] = z_u_l[1];
  ml[2] = 1.0;
  mr[0] = z_u_r[0];
  ...
  for (j = 0; j < 2; j++) {
    ...
    //  end
    anchorIdx = j * 3;
    b_rot[0] = ((rot[j << 2] * rot[j << 2] - rot[1 + (j << 2)] * rot[1 + (j << 2)])
                - rot[2 + (j << 2)] * rot[2 + (j << 2)]) + rot[3 + (j << 2)] *
      rot[3 + (j << 2)];
    b_rot[1] = 2.0 * (rot[j << 2] * rot[1 + (j << 2)] + rot[2 + (j << 2)] * rot
                      [3 + (j << 2)]);
    b_rot[2] = 2.0 * (rot[j << 2] * rot[2 + (j << 2)] - rot[1 + (j << 2)] * rot
                      [3 + (j << 2)]);
    b_rot[3] = 2.0 * (rot[j << 2] * rot[1 + (j << 2)] - rot[2 + (j << 2)] * rot
                      [3 + (j << 2)]);
    b_rot[4] = ((-(rot[j << 2] * rot[j << 2]) + rot[1 + (j << 2)] * rot[1 + (j <<
      2)]) - rot[2 + (j << 2)] * rot[2 + (j << 2)]) + rot[3 + (j << 2)] * rot[3
      + (j << 2)];
    b_rot[5] = 2.0 * (rot[1 + (j << 2)] * rot[2 + (j << 2)] + rot[j << 2] * rot
                      [3 + (j << 2)]);
    b_rot[6] = 2.0 * (rot[j << 2] * rot[2 + (j << 2)] + rot[1 + (j << 2)] * rot
                      [3 + (j << 2)]);
    b_rot[7] = 2.0 * (rot[1 + (j << 2)] * rot[2 + (j << 2)] - rot[j << 2] * rot
                      [3 + (j << 2)]);
    b_rot[8] = ((-(rot[j << 2] * rot[j << 2]) - rot[1 + (j << 2)] * rot[1 + (j <<
      2)]) + rot[2 + (j << 2)] * rot[2 + (j << 2)]) + rot[3 + (j << 2)] * rot[3
      + (j << 2)];
    for (i = 0; i < 3; i++) {
      A[(i + anchorIdx) + 6 * j] = 0.0;
      for (rankR = 0; rankR < 3; rankR++) {
        A[(i + anchorIdx) + 6 * j] += b_rot[i + 3 * rankR] * m[rankR + 3 * j];
      }
    }
    ...
    for (rankR = 0; rankR < 3; rankR++) {
      I[rankR + 3 * rankR] = 1;
      for (i = 0; i < 3; i++) {
        A[(i + anchorIdx) + 6 * (2 + rankR)] = -(double)I[i + 3 * rankR];
      }
    }

    anchorIdx = j * 3;
    for (i = 0; i < 3; i++) {
      b[i + anchorIdx] = -pos[i + 3 * j];
    }
  }
